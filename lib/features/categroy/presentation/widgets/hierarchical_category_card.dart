...
class HierarchicalCategoryCard extends StatelessWidget {
  ...
  Widget _buildAdminControls(BuildContext context) {
    return Positioned(
      ...
      child: PopupMenuButton(
        icon: Container(
          padding: EdgeInsets.all(ResponsiveUtils.getResponsiveSpacing(context, 6.0)),
          decoration: BoxDecoration(
  ...
  void _handleCategoryTap(BuildContext context) async {
    ...
    AppLogger.info('🎯 HierarchicalCategoryCard tapped: ${category.name} (ID: ${category.id})');
    ...
    if (hasSubcategories) {
      ...
      AppLogger.info('📂 Navigating to subcategories of: ${category.name} (ID: ${category.id})');
      cubit.navigateToSubcategories(category);
    } else {
      // Navigate to products page
      AppLogger.info('🛍️ Navigating to products for category: ${category.name} (ID: ${category.id})');
      context.push(
        AppRoutes.products,
        extra: {
  ...
  Future<bool> _checkForSubcategories(BuildContext context, int categoryId) async {
    try {
      final cubit = context.read<CategoriesCubit>();
      ...
      AppLogger.error('Error checking subcategories for category $categoryId: $e');
      return false;
    }
  }
