...
class CategoriesCubit extends Cubit<CategoriesState> {
  ...
  Future<void> navigateToSubcategories(Category parentCategory) async {
    AppLogger.info('🔍 Navigating to subcategories of: ${parentCategory.name} (ID: ${parentCategory.id})');
  ...
  Future<void> navigateBack() async {
    if (_navigationStack.isNotEmpty) {
      ...
      _currentParentId = _navigationStack.isNotEmpty ? _navigationStack.last.id : null;
  ...
  Future<bool> checkHasSubcategories(int categoryId) async {
    try {
      AppLogger.info('🔍 Checking for subcategories of category ID: $categoryId');
      ...
      return result.fold(
        ...
        (response) {
          ...
          AppLogger.info('✅ Category $categoryId has subcategories: $hasSubcategories (found ${response.categories.length})');
  ...
  Future<void> getCategoriesByParent(int parentId, {bool isInitialLoad = false}) async {
    if (_isFetching) {
      return;
    }
    ...
    try {
      ...
      result.fold(
        ...
        (response) {
          ...
          AppLogger.info('📦 Loaded ${newCategories.length} subcategories for parent $parentId:');
          for (final category in newCategories) {
            AppLogger.info('  - ${category.name} (ID: ${category.id}, ParentID: ${category.parentId})');
          }
  ...
  Future<void> createCategory(
    ...
    final result = await _createCategory(name, description, imageFile, parentId: parentId);
    result.fold(
  ...
  Future<void> updateCategory(
    ...
    emit(CategoriesFormSubmitting());
    final result = await _updateCategory(id, name, description, imageFile, parentId: parentId);
    result.fold(
      (failure) => emit(CategoriesFormError("Failed to update: $failure")),
      (voidcategory) {
        emit(CategoriesFormSuccess(Category(id: id, name: name, description: description, imagePath: imageFile.path, parentId: parentId)));
