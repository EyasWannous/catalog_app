...
class MultiImagePicker extends StatefulWidget {
  ...
  final Function(List<File> newImages, List<int> deletedAttachmentIds) onImagesChanged;
  final int maxImages;
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
            'You can only add up to ${widget.maxImages} images per product.',
          ),
          actions: [
            TextButton(
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
            'Please select an image smaller than ${widget.maxImageSizeMB}MB.',
          ),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
        ...
        if (mounted) {
          ...
          if (pickedFiles.length > remainingSlots) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  'Only $remainingSlots image(s) could be added (max ${widget.maxImages} total)',
                ),
                backgroundColor: Colors.orange,
                duration: const Duration(seconds: 3),
              ),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            imageItem.isLocal
                ? 'New image removed'
                : 'Existing image will be deleted',
          ),
